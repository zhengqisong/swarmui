$(function() {
	...
	pserviceid = $.getContextLoadParam("serviceid");
	pname = $.getContextLoadParam("name");
	
	var navigation_html = '<strong class="am-text-primary am-text-lg">首页</strong>'
		+' / <small><a href="javascript:$.contextLoad(\'/service/servicelist.html\')">配置管理</a></small>'
		+' / <small><a href="javascript:$.contextLoad(\'/service/userlist.html?serviceid='+pserviceid+'&name='+pname+'\')">配置授权管理('+pname+')</span></a></small>'
		+' / <small>新增</small> ';
	...
	$("#userrightForm").submit(function(e){
		...
		//$('body').scrollTop(0);
		var aj = $.ajax({    
			url:'/api/service/'+currentCluster.clusterid+'/setright/'+pserviceid,// 跳转到 action
		    contentType: "application/json;charset=utf-8",
		    type:'post',
		    data:JSON.stringify(jsonObj),
		    cache:false,
		    ...
		    success:function(data) {
		    	if(data.status != 0) {
		    		...
		    		$.contextLoad("/service/userlist.html?serviceid="+pserviceid+"&name="+pname);
		    	}
		    	return false;
		    },
		    error : function() {
