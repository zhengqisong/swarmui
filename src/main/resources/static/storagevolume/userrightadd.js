$(function() {
	...
	var navigation_html = '<strong class="am-text-primary am-text-lg">首页</strong>'
		+' / <small><a href="javascript:$.contextLoad(\'/storagevolume/storagevolumelist.html\')">配置管理</a></small>'
		+' / <small><a href="javascript:$.contextLoad(\'/storagevolume/userlist.html?volumeid='+pvolumeid+'&name='+pname+'\')">配置授权管理('+pname+')</span></a></small>'
		+' / <small>新增</small> ';
	...
	$("#userrightForm").submit(function(e){
		...
		//$('body').scrollTop(0);
		var aj = $.ajax({    
			url:'/api/storagevolume/'+currentCluster.clusterid+'/setright/'+pvolumeid,// 跳转到 action
		    contentType: "application/json;charset=utf-8",
		    type:'post',
		    data:JSON.stringify(jsonObj),
		    cache:false,
		    ...
		    success:function(data) {
		    	if(data.status != 0) {
		    		...
		    		$.contextLoad("/storagevolume/userlist.html?volumeid="+pvolumeid+"&name="+pname);
		    	}
		    	return false;
		    },
		    error : function() {
